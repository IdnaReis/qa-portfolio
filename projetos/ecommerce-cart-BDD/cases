...
Valide a funcionalidade do carrinho de compras garantindo que os produtos possam ser prolongados, removidos, atualizados e que o valor total seja calculado corretamente.
...
```
 ...
 ## Cenário: Removendo produto do carrinho
...
```
 ...
 ## 📌 Observações
Casos escritos em Gherkin para facilitar a comunicação entre QA, Dev e Negócio.

Podem ser documentados em ferramentas como Qase ou integrados em pipelines de automação (ex.: Cucumber).

Próximos passos: Implementar automação dos cenários com Selenium, Cypress ou testes de API com Postman.

 ## 🛠️ Ferramentas sugeridas
 
Notion→ Documentação dos testes

Trello → Gestão das tarefas

Git → Versionamento de scripts de automação

Pepino → Automação dos cenários BDD.
